...
<div class="x_panel">
    ...
    <div class="x_content">
        ...
        <div class="row" id="divCatalogoCostCenter">
            <div class="col-sm-12">
                            ...
                            <div class="text-right">
                                <!--You can add col-lg-12 if you want -->
                                    ...
                                    <button class="btn btn-primary" type="button"
                                        onclick="javascript:UpdateContenedor('AddCostCenter');">
                                        <span class="glyphicon glyphicon-plus" id="icon_enabled"
                                            style="color:white;"></span> NEW COST CENTER
                                    </button>
                            ...
                            <thead>
                                <tr role="row">
                                    ...
                                    <th class="sorting" tabindex="0" aria-controls="datatable-buttons" rowspan="1"
                                        ...
                                        aria-label="Mail: activate to sort column ascending">Name Center</th>
...
<table id="tablaC" class="table table-striped" cellspacing="0" width="100%"
        ...
        <!--Editar  usuario-->
        <div id="divEditarCostCenter" style="display:none;">
            <br>

            <div class="col-md-6">
                    <div id="divCombo1" class="form-group">
                    <label class="control-label mb-10 text-left">Department * </label>
                    <select id="Department2" class="form-control validarCombo">
                        <option value="Select Option" required="required" selected="selected">Select Option
                        </option>
                ...
                <div id="divCombo2" class="form-group">
                    ...
                    <input type="text" class="form-control validar"
                        ...
                        name="CenterCost" id="CostCenter" placeholder="Cost Center">

                    <input type="text" name="CostCenter" id="ID" style="display: none;">
            ...
            <div class="form-group row form-group row pull-left">
                <div class="col-md-12">
                    ...
                    <input type="button" value="Update" class="btn btn-success" id="saveCC"
...
$("#Department").change(function() {
    if (true) {
        ...
        $('#seeAll').show();
        llenarObjCentroCostos($("#Department").val(), "");
    }
});
...
function Filldeps() {
    ...
    $('#Department2').empty();
    $('#Department').empty();
    dep = ' <option value="Select Option" selected disabled>Select Option</option>';
    $(ArrayDep).each(function(i) {
        dep += '<option value="' + this.ID + '">' + this.Departamento + '</option>';
    });
    $('#Department').html(dep);
    $('#Department2').html(dep);
...
var ObjCentroCostos = "";
/* DATA TABLES */
var Tabla_CentroCostos;
$(document).ready(function() {

    init_DataTables();
    //llenarObjCentroCostos();
});
...
function init_DataTables() {
    ...
    var handleDataTableButtons = function() {
        ...
        if ($("#datatable-buttons").length) {
            Tabla_CentroCostos = $("#datatable-buttons").DataTable({
                dom: "Blfrtip",
                ...
                buttons: [{
                        //extend: 'excelHtml5',
                        footer: true,
                        title: 'Cost_Center' + today,
                        filename: 'Cost_Center' + today,
                        exportOptions: {
                            columns: "0,1,2,3",
                            orthogonal: 'export'
                        },
                    ...
                    },
                    {
                        ...
                        customize: function(doc) {
                           ...
                           age = Tabla_CentroCostos.column(3).data().toArray();
                           for (var i = 0; i < age.length; i++) {
                               var text = age[i].substring(35, 43);
                               if (text != "DISABLED") {
                                   doc.content[1].table.body[i + 1][3].fillColor = '#72DB3A';
                               } else {
                                   doc.content[1].table.body[i + 1][3].fillColor = '#FF0000';
                               }
                           }
                       }
...
function llenarObjCentroCostos(id, all) {
    ...
    $.ajax({
        ...
        success: function(data) {
            ...
            ObjCentroCostos = data.lstCentroCostos;
...
function llenarTabla() {
    ...
    for (var i = 0; i < ObjCentroCostos.length; i++) {

        if (ObjCentroCostos[i].status == "0") {
        ...
        if (pos == 0) {
            Tabla_CentroCostos.row.add([
                String(ObjCentroCostos[i].id),
                String(ObjCentroCostos[i].idDepartamento),
                String(ObjCentroCostos[i].nameCentro),
                String(colorStatus),
                String('<button type="button" onclick="MostrarDivEditar(' + i + ',' + ObjCentroCostos[i].id +
                    ')" class="boton_info"> EDIT</button>'),
                String('<button type="button" onclick="statusCostCenter(' + i + ',' + ObjCentroCostos[i].id +
    ...
    Tabla_CentroCostos.column('1:visible').order('desc').draw()
...
function MostrarDivEditar(pos, id) {
    console.log("cod");
    $('#divEditarCostCenter').show();
    $('#divCatalogoCostCenter').hide();
    $("#Department2").val($("#Department").val())
    $("#CostCenter").val(ObjCentroCostos[pos].nameCentro.trim());
    $("#ID").val(ObjCentroCostos[pos].id);

}

function UpdateCentroCostos() {
    try {
        var ID = $('#ID').val();
        var CostCenter = $('#CostCenter').val();
        var idDepartamento = $('#Department2').val();
        ...
        var para = {
            "ID": ID,
            "nameCentro": CostCenter,
            "idDepartamento":idDepartamento,

        };
        swal({
                title: "Are you sure?",
                text: "THIS COST CENTER WILL BE UPDATED!",
                icon: "warning",
                buttons: true,
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    var sURL = IPWS +"WsUpdateCentroCostos";
                    $.ajax({
                        url: sURL,
                        data: para,
                        dataType: "json",
                        ...
                        success: function(data) {
                            ...
                            if (data.Status == "OK") {
                                swal("OK!", "COST CENTER UPDATED SUCCESSFULLY", "success");
...
function UpdateDisabled(pos, id) {
    console.log("sdkfndjsonf");
    swal({
            title: "SURE YOU WANT DISABLED THIS COST CENTER?",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        })
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.Status == "OK") {
                            swal("GOOD JOB!", "COST CENTER DISABLED SUCCESSFULLY!", "success")
...
function statusCostCenter(pos, id) {
    console.log("dis");
    if (ObjCentroCostos[pos].status == "0") {
...
function UpdateEnabled(pos, id) {
    swal({
            title: "SURE YOU WANT ENABLED THIS COST CENTER?",
            icon: "warning",
            buttons: true,
            dangerMode: true,
        })
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.Status == "OK") {
                            swal("GOOD JOB!", "COST CENTER ENABLED SUCCESSFULLY!", "success")
...
$('#seeAll').click(function() {
    if (ocultarelementos == 0) {
        ...
        llenarObjCentroCostos($("#Department").val(), "5");
    } else {
        ocultarelementos = 0;
...
pdfMake.createPdf(documentDefinition).download('COSTCENTER_REPORT_"'+moment(new Date()).subtract(5, 'hours').format('MMMM,DD, YYYY')+'".pdf');
}
...
var arrayN = ['ID', 'DEPARTMENT','COSTCENTER', 'STATUS'];
var detail = [
    [
        {
            text: 'ID',
...
for (let index = 0; index < ObjCentroCostos.length; index++) {
    if (id) {
        datos.push({
            text: ObjCentroCostos[index].id,
            style: 'tableText'
        });
    }else{
        datos.push({
    ...
    if (department) {
        datos.push({
            text: ObjCentroCostos[index].idDepartamento,
            style: 'tableText'
        });
    }else{
        datos.push({
    ...
    if (cost) {
        datos.push({
            text: ObjCentroCostos[index].nameCentro,
            style: 'tableText'
        });
    }else{
        datos.push({
    ...
    if (status) {
        var estatus="";
        if (ObjCentroCostos[index].status == 0) {
            estatus = "ENABLED";

            htmlBotonStatus = 'boton_danger';
            title = "DISABLED";
        } else if (ObjCentroCostos[index].status != 0) {
            htmlBotonStatus = 'boton_success';
            title = "ENABLED";
            estatus = "DISABLED";
        }
...
function descargarExcel(){
    ...
    link.download = "COSTCENTER_REPORT_"+fecha+".xls";
...
var status = document.getElementById("STATUS").checked;
    ...
    for (var i = 0; i < ObjCentroCostos.length; i++) {
        ...
        if(department){
            ...
            $('#Department option:selected').text() + "</td>");
        }else{
            tr.append(
        ...
        if(cost){
            ...
            ObjCentroCostos[i].nameCentro + "</td>");
        }else{
            tr.append(
...
<?php /**PATH C:\xampp\htdocs\TANKFARM\Laravel\resources\views/CostCenter.blade.php ENDPATH**/ ?>