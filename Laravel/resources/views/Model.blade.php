    ...
    for (var i = 0; i < ObjModelo.length; i++) {
        ...
        if (pos == 0) {
            Tabla_Modelos.row.add([
                String(quitarCaracteres(ObjModelo[i].ModelId)),
                String(quitarCaracteres(ObjModelo[i].ModelDesc)),
                String(colorStatus),
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            if (data.Status == "OK") {
                                swal.fire("OK!", "MODEL UPDATED SUCCESSFULLY", "success");
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.Status == "OK") {
                            swal.fire("GOOD JOB!", "MODEL DISABLED SUCCESSFULLY!", "success");
                            llenarObjModels("", "");
                            UpdateContenedor('Model');
                        }
...
for (let index = 0; index < ObjModelo.length; index++) {
    if (id) {
        datos.push({
            text: quitarCaracteres(ObjModelo[index].ModelId),
            style: 'tableText'
        });
    }else{
        datos.push({
    ...
    if (model) {
        datos.push({
            text: quitarCaracteres(ObjModelo[index].ModelDesc),
            style: 'tableText'
        });
    }else{
        datos.push({
...
var status = document.getElementById("STATUS").checked;
    ...
    for (var i = 0; i < ObjModelo.length; i++) {
        ...
        if(id){
            ...
            quitarCaracteres(ObjModelo[i].ModelId) + "</td> ");
        }else{
            tr.append(
        ...
        if(model){
            ...
            quitarCaracteres(ObjModelo[i].ModelDesc) + "</td>");
        }else{
            tr.append(
