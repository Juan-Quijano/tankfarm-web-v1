...
function llenarTabla() {
    ...
    for (var i = 0; i < ObjDepartment.length; i++) {
        ...
        if (pos == 0) {
            Tabla_Department.row.add([
                String(quitarCaracteres(ObjDepartment[i].ID)),
    ...
    Tabla_Department.column('1:visible').draw()
}
                ...
                if (willDelete) {
                    ...
                    $.ajax({
                        ...
                        success: function(data) {
                            ...
                            if (data.Status == "OK") {
                                swal.fire("OK!", "DEPARTMENT UPDATED SUCCESSFULLY", "success");
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.Status == "OK") {
                            swal.fire("GOOD JOB!", "DEPARTMENT DISABLED SUCCESSFULLY!", "success")
        ...
        .then((willDelete) => {
            if (willDelete) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        if (data.Status == "OK") {
                            swal.fire("GOOD JOB!", "DEPARTMENT ENABLED SUCCESSFULLY!", "success")
...
pdfMake.createPdf(documentDefinition).download('DEPARTMENT_REPORT_"'+moment(new Date()).subtract(5, 'hours').format('MMMM,DD, YYYY')+'".pdf');
}
...
var arrayN = ['ID', 'DEPARTMENT', 'STATUS'];
var detail = [
    [
        {
            text: 'ID',
...
for (let index = 0; index < ObjDepartment.length; index++) {
    if (id) {
        datos.push({
            text: quitarCaracteres(ObjDepartment[index].ID),
            style: 'tableText'
        });
    }else{
        datos.push({
    ...
    if (department) {
        datos.push({
            text: ObjDepartment[index].Departamento,
            style: 'tableText'
        });
    }else{
        datos.push({
    ...
    if (status) {
        var estatus="";
        if (ObjDepartment[index].Status == 1) {
            estatus = "ENABLED";

            htmlBotonStatus = 'boton_danger';
            title = "DISABLED";
        } else if (ObjDepartment[index].Status != 1) {
            htmlBotonStatus = 'boton_success';
            title = "ENABLED";
            estatus = "DISABLED";
        }
...
function descargarExcel(){
    ...
    link.download = "DEPARTMENT_REPORT_"+fecha+".xls";
...
var status = document.getElementById("STATUS").checked;
    ...
    for (var i = 0; i < ObjDepartment.length; i++) {
        ...
        if(id){
            ...
            quitarCaracteres(ObjDepartment[i].ID) + "</td> ");
        }else{
            tr.append(
        ...
        if(department){
            ...
            ObjDepartment[i].Departamento + "</td>");
        }else{
            tr.append(
