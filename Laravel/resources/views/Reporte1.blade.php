<script src="{{url('/')}}/vendors/loading/loadingoverlay.min.js"></script>
                ...
                <div class="text-left">
                    ...
                    <button type="button" class="btn btn-primary " onclick="openNav()"> <span class="" id="icon_enabled"
                            ...
                            <select  name="SelectTable" id="SelectTable" class="btn btn-primary" style='height: 38px; display:none'
                                ...
                                <option value="1">DIESEL CONSUMPTIONS TF</option>
                                <option value="2">TOTALS CONSUMPTIONS TF</option>
                        ...
                        <ul class="nav nav-tabs" id="myTabGrafica" role="tablist">
                            ...
                            <li class="nav-item">
                                <a class="nav-link" id="profile-tab" data-toggle="tab" href="#profile" role="tab"
                                    aria-controls="profile" aria-selected="false">Totals Consumption</a>
                            </li>
...
<div class="tbl_container1">
    <table id="tablaC" class="table table-striped" cellspacing="0" width="100%" style="border: 1px solid black;display:none">
        ...
        <tr>
            ...
            src="{{url('/')}}/images/tdcon_gris.png"></th>
        </tr>
        <tr>
            <th colspan=5
                style="text-align: left;background: #233461;font-size: 14px;font-family:BMWTypeCondensedRegular;font-weight: bold;">
...
<div class="tbl_container1">
    <table id="tablaC1" class="table table-striped" cellspacing="0" width="100%"
        ...
        <tr > 
            ...
            <img style="margin-right: -69px;text-align: right; "src="{{url('/')}}/images/logogrisM.png"></th>    
...
$(document).ready(function() {
    ...
    FillVehicleTag();
    FiltrosDespachos("", "", "", "", "", hrInicioTT, fechaFinal);
    gerenarGrapoh2();
    $('#PanelDatasheet').hide();
});
...
function showChart(show) {
    ...
    if (show == 0 ){
        ...
        $('#PanelDatasheet').show();
        $("#SelectTable").show();
        if($('.nav-tabs .active').text()=='Totals Consumption'){
...
function llenarTabla2() {
    ...
    for (var i = 0; i < despachos.length; i++) {
        TablaX1.row.add([
            String(((despachos[i].xDepartamento == "" || despachos[i].xDepartamento == null) ? "" : despachos[i]
                .xDepartamento)),
            String(((despachos[i].xCentroCostos == "" || despachos[i].xCentroCostos == null) ? "" : despachos[i]
                .xCentroCostos)),
            String(((despachos[i].xCliente == "" || despachos[i].xCliente == null) ? "" : despachos[i]
                .xCliente)),
            String(((despachos[i].xFecha  == "" || despachos[i].xFecha == null) ? "" :despachos[i].xFecha +" "+ despachos[i].xHoraDespacho)),
            String(((despachos[i].xProducto == "" || despachos[i].xProducto == null) ? "" : despachos[i]
                .xProducto)),
            String(((despachos[i].can == "" || despachos[i].can == null) ? "" : despachos[i].can)),
            String(((despachos[i].xNombreChofer == "" || despachos[i].xNombreChofer == null) ? "" : despachos[i].xNombreChofer)),
            String(((despachos[i].xPlacas == "" || despachos[i].xPlacas == null) ? "" : despachos[i].xPlacas)),
        ]);
    }
...
function FiltrosDespachos(LastConsum, Estacion, Company, DriverTag, VehicleTag, Department, Cost, StartDate, EndDate) {
    $.LoadingOverlay("show");
   /*
    ...
    */
    ...
    if (LastConsum == "0" && Company == "" && VehicleTag == "" && DriverTag == "" && Department == "" && Cost == "" &&
        StartDate == "" && EndDate == "") {
        swal.fire("Error", "No filter selected", "error");
    } else {
        var para = {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    if (despachos.length > 0) {
                        ArrayFechasTF = despachos[0].ArrayFechasTF;
                        ArrayDieselTF = despachos[0].ArrayDieselTF;
                        ArrayTrnTF = despachos[0].ArrayTrnTF;
...
function mostrarParametrosTT() {
    ...
    if ($("#hrinicio").val() == "") {
        ...
        $('#fechainicioTT').text($("#hrinicio").val());
    }
    if ($("#hrfinal").val() == "") {
        $('#fechafinTT').text("");
    } else {
        $('#fechafinTT').text($("#hrfinal").val());
    }
    if (Field == "") {
        $('#fieldTT').text("");
    } else {
...
function GetxReporteFiltro() {
    ...
    GetxReporte( $("#hrinicio").val(), $("#hrfinal").val(), campoTipoTT, combustibleTT);
    $("#encaTT").empty();
...
function openNav() {
    ...
    if (ventanaSeleccionada == "Totals Consumption") {
...
function descargarExcel(){
    ...
    link.download = "FUEL_CONSUMPTIONS_REPORT_"+fecha+"_TANKFARM.xls";
...
function exceller1() {
    ...
    var link = document.createElement("a");
    var fecha=moment(new Date()).subtract(5, 'hours').format('DD [de] MMMM [de] YYYY');
    link.download = "TOTALS_CONSUMPTIONS_REPORT_"+fecha+"_TANKFARM.xls";
    link.href = uri + base64(format(template, ctx))
    link.click(); 
...
function generatePDF() {
    ...
    pdfMake.createPdf(documentDefinition).download('FUEL_CONSUMPTION_REPORT_"'+moment(new Date()).subtract(5, 'hours').format('DD [de] MMMM [de] YYYY')+'"_TANKFARM.pdf');
}
...
function getDatosBMW() {
    ...
    for (let index = 0; index < despachos.length; index++) {
        if (depa) {
            datos.push({
                text: despachos[index].xDepartamento,
                style: 'tableText'
            });
        }else{
            datos.push({
        ...
        if (cos) {
            datos.push({
                text: despachos[index].xCentroCostos,
                style: 'tableText'
            });
        }else{
            datos.push({
        ...
        if (dat) {
            datos.push({
                text: despachos[index].ArrayFechasTF[index],
                style: 'tableText'
            });
        }else{
            datos.push({
        ...
        if (fue) {
            datos.push({
                text: despachos[index].xProducto,
                style: 'tableText'
            });
        }else{
            datos.push({
        ...
        if (driv) {
            datos.push({
                text: despachos[index].xNombreChofer,
                style: 'tableText'
            });
        }else{
            datos.push({
